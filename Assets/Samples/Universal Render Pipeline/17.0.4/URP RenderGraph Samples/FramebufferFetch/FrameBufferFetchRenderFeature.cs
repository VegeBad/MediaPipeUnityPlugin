...
public class FrameBufferFetchRenderFeature : ScriptableRendererFeature
{
	class FrameBufferFetchPass : ScriptableRenderPass
	{
		...
		{
			context.cmd.DrawProcedural(Matrix4x4.identity, data.material, data.useMSAA ? 1 : 0, MeshTopology.Triangles,
				3, 1, null);
		}

		private void FBFetchPass(RenderGraph renderGraph, ContextContainer frameData, TextureHandle source,
			TextureHandle destination, bool useMSAA)
		{
		...
		public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
		{
			...
			if (destinationDesc.msaaSamples == MSAASamples.None || RenderGraphUtils.CanAddCopyPassMSAA())
			{
				...
				FBFetchPass(renderGraph, frameData, source, fbFetchDestination,
				...
				renderGraph.AddCopyPass(fbFetchDestination, source,
					passName: "Copy Back FF Destination (also using FBF)");
			}
			else
			{
