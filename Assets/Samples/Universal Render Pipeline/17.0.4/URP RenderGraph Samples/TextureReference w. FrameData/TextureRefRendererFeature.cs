...
public class TextureRefRendererFeature : ScriptableRendererFeature
{
	...
	class UpdateRefPass : ScriptableRenderPass
	{
		...
		public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
		{
			foreach (var mat in m_DisplayMaterials)
			{
				...
				using (var builder =
				       renderGraph.AddRasterRenderPass<PassData>($"UpdateRefPass_{mat.name}", out var passData))
				{
	...
	class CopyBackRefPass : ScriptableRenderPass
	{
		...
		public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
		{
			...
			renderGraph.AddBlitPass(texRef.texture, resourceData.activeColorTexture, Vector2.one, Vector2.zero,
				passName: "Blit Back Pass");
		}
	}
