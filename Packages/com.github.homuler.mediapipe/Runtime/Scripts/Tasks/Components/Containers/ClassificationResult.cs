...
namespace Mediapipe.Tasks.Components.Containers
{
	...
	public readonly struct ClassificationResult
	{
		...
		public static ClassificationResult Alloc(int capacity) =>
			new ClassificationResult(new List<Classifications>(capacity), null);
...
#pragma warning disable IDE0004 // for Unity 2020.3.x
			destination = new ClassificationResult(classifications,
				source.HasTimestampMs ? (long?)source.TimestampMs : null);
#pragma warning restore IDE0004 // for Unity 2020.3.x
		...
		internal static void Copy(NativeClassificationResult source, ref ClassificationResult destination)
		{
			...
			destination = new ClassificationResult(classificationsList,
				source.hasTimestampMs ? (long?)source.timestampMs : null);
		}
