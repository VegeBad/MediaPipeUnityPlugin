...
namespace Mediapipe.Tasks.Vision.Core
{
	...
	public class BaseVisionTaskApi : IDisposable
	{
		...
		protected BaseVisionTaskApi(
			...
			Tasks.Core.TaskRunner.PacketsCallback packetsCallback) : this(graphConfig, runningMode, null,
			packetsCallback)
		{
		}
			...
			Tasks.Core.TaskRunner.PacketsCallback packetsCallback)
		{
			...
			{
				_taskRunner =
					Tasks.Core.TaskRunner.Create(graphConfig, gpuResources, callbackId, nativePacketsCallback);
			}
			else
			{
