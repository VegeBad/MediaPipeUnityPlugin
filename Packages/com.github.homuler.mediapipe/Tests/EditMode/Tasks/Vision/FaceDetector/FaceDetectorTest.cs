...
namespace Mediapipe.Tests.Tasks.Vision
{
	public class FaceDetectorTest
	{
		...
		private readonly Lazy<TextAsset> _faceDetectorModel =
			new Lazy<TextAsset>(() =>
				AssetDatabase.LoadAssetAtPath<TextAsset>($"{_ResourcePath}/blaze_face_short_range.bytes"));
		...
		{
			var options =
				new FaceDetectorOptions(new BaseOptions(BaseOptions.Delegate.CPU,
		...
		public void Detect_ShouldReturnAnEmptyResult_When_ImageIsEmpty()
		{
			var options =
				new FaceDetectorOptions(
					new BaseOptions(BaseOptions.Delegate.CPU, modelAssetBuffer: _faceDetectorModel.Value.bytes),
